...
function ComparisonPlanCard({ id, name, priceMargin, baseMargins, thresholds }: ComparisonPlanCardProps) {
  ...
  // Get margin color (using shades of primary/accent)
  const getMarginColor = (margin: number | undefined) => {
    if (!margin || isNaN(margin)) return 'bg-[#4A7EC7]'
    if (margin >= 0.5) return 'bg-[#1E3A5F]'
    if (margin >= 0.4) return 'bg-[#2A5080]'
    if (margin >= 0.3) return 'bg-[#3668A0]'
    return 'bg-[#4A7EC7]'
  }
        ...
        <div className="p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between border-t border-border pt-2">
              ...
              <span className="font-medium text-red-500">{formatCurrency(baseMargins?.totalCost)}</span>
            </div>
            <div className="flex justify-between border-t border-border pt-2">
          ...
          {thresholds && (
            <div className="mt-4 pt-4 border-t border-border">
              ...
              <div className="space-y-2 text-xs">
                {thresholds.targetMarginEvalPrice !== null && (
                  <div className="flex justify-between">
                    <span className="text-text_secondary">Evaluation Price</span>
                    <span>{formatCurrency(thresholds.targetMarginEvalPrice)}</span>
                  </div>
                )}
              </div>
            </div>
